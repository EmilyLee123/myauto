...
from testFile.elementAndText import config_element
...
class Page():
    ...
    def find_elements(self,page,elements):
        ...
        try:
            ...
            if type == "ID" or type == "id" or type == "Id":
                ...
                error = "@{}模块下{}元素类型参数错误！@".format(page, elements)
                logger.error(error)

        except KeyError as e:
            logger.error("@加载{}模块{}元素不匹配@".format(page, elements))
            logger.error(e)
            raise e
    ...
    def element_type_input(self, page, element, name="value"):
        """根据元素属性获取输入值"""
        attribute = self.find_element(page, element).get_attribute(name)
        self.wait_time(0.5)
        return  attribute
    ...
    def window_to_old(self, page='old_page'):
        """该方法的前提是默认打开了新窗口"""
        handles = self.driver.window_handles
        lens = len(handles)
        if page == "old_page":
